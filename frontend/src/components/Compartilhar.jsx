...
export default function Compartilhar({ denunciaId, titulo, initialCount = 0 }) {
  ...
  return <div className="rc-share" ref={menuRef} onKeyDown={event => { if (event.key === "Escape" && open) { event.stopPropagation(); setOpen(false); menuRef.current?.querySelector("button")?.focus(); } }}>
    <button className="btn btn-outline-secondary btn-sm" onClick={toggleMenu} aria-expanded={open}>
      <i aria-hidden="true" className="bi bi-share me-1" /> Compartilhar <span className="rc-share-count">{count}</span>
    </button>
    {open && <div className="rc-share-menu rc-share-menu-expanded">
      <div className="rc-share-tabs" role="group" aria-label="Opções de compartilhamento">
        <button className={activeTab === 'share' ? 'is-active' : ''} aria-pressed={activeTab === 'share'} onClick={() => setActiveTab('share')}><i aria-hidden="true" className="bi bi-send" /> Compartilhar</button>
        <button className={activeTab === 'history' ? 'is-active' : ''} aria-pressed={activeTab === 'history'} onClick={() => { setActiveTab('history'); if (!historyLoaded) carregar(); }}><i aria-hidden="true" className="bi bi-clock-history" /> Histórico <span>{count}</span></button>
      </div>

      {activeTab === 'share' ? <div className="rc-share-tab-panel" role="region" aria-label="Compartilhar denúncia">
        <div className="rc-share-title">Compartilhar denúncia</div>
        {user ? <>
          <textarea aria-label="Mensagem do compartilhamento (opcional)" maxLength="255" rows="2" value={comentario} onChange={event => setComentario(event.target.value)} placeholder="Adicione uma mensagem (opcional)" />
          <div className="rc-share-options">
            <button disabled={sharing} onClick={() => escolher('whatsapp')}><i aria-hidden="true" className="bi bi-whatsapp" /> WhatsApp</button>
            <button disabled={sharing} onClick={() => escolher('facebook')}><i aria-hidden="true" className="bi bi-facebook" /> Facebook</button>
            <button disabled={sharing} onClick={() => escolher('copy')}><i aria-hidden="true" className="bi bi-link-45deg" /> Copiar link</button>
          </div>
        </> : <p className="rc-share-login">Entre na sua conta para registrar um compartilhamento.</p>}
      </div> : <div className="rc-share-history rc-share-tab-panel" role="region" aria-label="Histórico de compartilhamentos">
        {shares.length === 0 ? <small>Ainda não há compartilhamentos.</small> : recentShares.map(share => <div className="rc-share-history-item" key={share.id}>
          ...
          {Number(share.userId) === Number(user?.id) && <button className="rc-share-remove" title="Remover do meu histórico" aria-label="Remover do meu histórico" onClick={() => remover(share)}><i aria-hidden="true" className="bi bi-trash" /></button>}
        </div>)}
        {shares.length > 0 && <>
