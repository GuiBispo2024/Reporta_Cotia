...
export default function ImageCarousel({ images = [], fallback, alt = 'Imagem da denúncia', compact = false, setorResponsavel, categoria }) {
  ...
  return <div className={`rc-image-carousel ${compact ? 'is-compact' : ''}`}>
    ...
    {available.length > 1 && <><button type="button" className="is-prev" onClick={() => move(-1)} aria-label="Imagem anterior"><i aria-hidden="true" className="bi bi-chevron-left" /></button><button type="button" className="is-next" onClick={() => move(1)} aria-label="Próxima imagem"><i aria-hidden="true" className="bi bi-chevron-right" /></button><span className="rc-carousel-counter" aria-live="polite">{active + 1}/{available.length}</span><div className="rc-carousel-dots">{available.map((_, index) => <button type="button" aria-label={`Ver imagem ${index + 1}`} aria-current={index === active ? "true" : undefined} className={index === active ? 'is-active' : ''} onClick={() => setActive(index)} key={index} />)}</div></>}
  </div>;
}
