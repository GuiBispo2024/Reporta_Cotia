...
export default function Moderacao() {
  ...
  const dialogRef = useDialogAccessibility(!!selectedReport, () => setSelectedReport(null));
    ...
    <div className="rc-page">
      ...
      <main tabIndex={-1} id="main-content" className="container py-4 flex-grow-1">
        ...
        <div className="rc-moderation-filter mb-4">
          <div><i aria-hidden="true" className="bi bi-funnel" /><div><strong>Filtrar denúncias</strong><small>Exiba apenas o status que deseja analisar.</small></div></div>
        ...
        {loading && <div className="text-center"><div role="status" aria-label="Carregando" className="spinner-border text-primary" /></div>}
              ...
              <article className="card rc-card h-100">
                ...
                <div className="card-body">
                  ...
                  {canReviewCensorship && (d.tituloOriginal || d.descricaoOriginal) && <div className="rc-censorship-review">
                    <strong><i aria-hidden="true" className="bi bi-eye" /> Revisão de conteúdo automático</strong>
                  ...
                  <button className="btn btn-outline-primary btn-sm w-100 mb-2" onClick={() => setSelectedReport({ ...d, queue: 'pending' })}><i aria-hidden="true" className="bi bi-eye me-1" />Ver detalhes da denúncia</button>
                  {canReviewModeration ? <><label htmlFor={`${accessibilityId}-field-1-${d.id}`} className="form-label small fw-semibold mt-2">Motivo da rejeição <span className="text-danger">(obrigatório para rejeitar)</span></label>
                    <textarea id={`${accessibilityId}-field-1-${d.id}`} className="form-control form-control-sm" rows="2" maxLength="1000" placeholder="Explique o que o cidadão pode corrigir..." value={motivos[d.id] || ''} onChange={e => setMotivos(prev => ({ ...prev, [d.id]: e.target.value }))} />
                    <div className="d-flex gap-2 mt-3"><button className="btn btn-success flex-fill" onClick={() => moderar(d.id, "aprovada")}>✅ Aprovar</button><button className="btn btn-danger flex-fill" disabled={!motivos[d.id]?.trim()} onClick={() => moderar(d.id, "rejeitada")}>❌ Rejeitar</button></div></> : <p className="rc-permission-note"><i aria-hidden="true" className="bi bi-eye" /> Acesso somente para consulta.</p>}
                  {canViewAudit && <button className="btn btn-outline-secondary btn-sm mt-3" onClick={() => navigate(`/moderacao/denuncia/${d.id}/historico`)}><i aria-hidden="true" className="bi bi-clock-history me-1" />Histórico de alterações</button>}
                </div>
              </article>
            </div>
          ))}
              ...
              >
                ...
                <div className="rc-filter-card">
                  ...
                  <p className="small text-muted mt-2 mb-2">
                    <i aria-hidden="true" className="bi bi-person-circle me-1" />
                  ...
                  <button className="btn btn-outline-primary btn-sm w-100 mt-2" onClick={() => setSelectedReport({ ...d, queue: 'approved' })}><i aria-hidden="true" className="bi bi-eye me-1" />Ver detalhes e atualizar</button>

                  <label htmlFor={`${accessibilityId}-field-2-${d.id}`}
                    className="form-label mt-3 fw-semibold"
                  >
                    Andamento
                  </label>

                  <select id={`${accessibilityId}-field-2-${d.id}`} disabled={!canUpdateResolution}
                  ...
                  <label htmlFor={`${accessibilityId}-field-3-${d.id}`} className="form-label mt-3 fw-semibold">Setor responsável</label>
                  <select id={`${accessibilityId}-field-3-${d.id}`} className="form-select" disabled={!canUpdateResolution} value={resolutionDetails[d.id]?.setorResponsavel || ''} onChange={event => setResolutionDetails(current => ({ ...current, [d.id]: { ...current[d.id], setorResponsavel: event.target.value } }))}>
      ...
      {selectedReport && <div className="rc-moderation-detail-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setSelectedReport(null); }}>
        <section ref={dialogRef} tabIndex={-1} className="rc-moderation-detail" role="dialog" aria-modal="true" aria-labelledby="moderation-detail-title">
          <header className="rc-moderation-detail-header">
            ...
            <button type="button" onClick={() => setSelectedReport(null)} aria-label="Fechar detalhes"><i aria-hidden="true" className="bi bi-x-lg" /></button>
          </header>
          <div className="rc-moderation-detail-grid">
            <div className="rc-moderation-detail-content">
              ...
              <dl className="rc-moderation-metadata">
                <div><dt><i aria-hidden="true" className="bi bi-geo-alt" /> Localização</dt><dd>{selectedReport.localizacao}</dd></div>
                <div><dt><i aria-hidden="true" className="bi bi-person" /> Publicado por</dt><dd>{selectedReport.User?.username || 'Usuário não identificado'}</dd></div>
                <div><dt><i aria-hidden="true" className="bi bi-calendar3" /> Enviado em</dt><dd>{new Date(selectedReport.createdAt).toLocaleString('pt-BR')}</dd></div>
              </dl>
            </div>
            <aside className="rc-moderation-detail-actions">
              ...
              {selectedReport.queue === 'approved' ? <>
                ...
                <label htmlFor={`${accessibilityId}-field-4`} className="form-label fw-semibold mt-3">Andamento</label>
                <select id={`${accessibilityId}-field-4`} className="form-select" disabled={!canUpdateResolution} value={resolutionDetails[selectedReport.id]?.resolucaoStatus || selectedReport.resolucaoStatus || 'aberta'} onChange={event => setResolutionDetails(current => ({ ...current, [selectedReport.id]: { ...current[selectedReport.id], resolucaoStatus: event.target.value } }))}><option value="aberta">Aberta</option><option value="em_andamento">Em andamento</option><option value="resolvida">Resolvida</option></select>
                <label htmlFor={`${accessibilityId}-field-5`} className="form-label fw-semibold mt-3">Setor responsável</label>
                <select id={`${accessibilityId}-field-5`} className="form-select" disabled={!canUpdateResolution} value={resolutionDetails[selectedReport.id]?.setorResponsavel || ''} onChange={event => setResolutionDetails(current => ({ ...current, [selectedReport.id]: { ...current[selectedReport.id], setorResponsavel: event.target.value } }))}><option value="">Selecione um setor</option>{SETORES.map(setor => <option value={setor} key={setor}>{setor}</option>)}</select>
                {canUpdateResolution && <button className="btn btn-primary w-100 mt-3" disabled={savingResolution === selectedReport.id || !hasResolutionChanges(selectedReport)} onClick={() => atualizarResolucao(selectedReport.id)}><i aria-hidden="true" className="bi bi-check2-circle me-1" />{savingResolution === selectedReport.id ? 'Salvando...' : 'Salvar mudanças'}</button>}
                ...
                <textarea id={`${accessibilityId}-field-6`} className="form-control" rows="4" maxLength="1000" placeholder="Explique o que o cidadão pode corrigir..." value={motivos[selectedReport.id] || ''} onChange={event => setMotivos(current => ({ ...current, [selectedReport.id]: event.target.value }))} />
                <div className="d-grid gap-2 mt-3"><button className="btn btn-success" onClick={() => moderar(selectedReport.id, 'aprovada')}><i aria-hidden="true" className="bi bi-check-circle me-1" />Aprovar denúncia</button><button className="btn btn-danger" disabled={!motivos[selectedReport.id]?.trim()} onClick={() => moderar(selectedReport.id, 'rejeitada')}><i aria-hidden="true" className="bi bi-x-circle me-1" />Rejeitar denúncia</button></div>
              </> : <p className="rc-permission-note"><i aria-hidden="true" className="bi bi-eye" /> Você possui acesso somente para consultar esta denúncia.</p>}
            </aside>
          </div>
        </section>
      </div>}
