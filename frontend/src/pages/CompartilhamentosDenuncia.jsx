    ...
    <main tabIndex={-1} id="main-content" className="container py-4 flex-grow-1">
      <section className="rc-history-page mx-auto">
        ...
        : <div className="rc-history-list">{shares.map(share => <article className="rc-history-item" key={share.id}>
          ...
          {Number(share.userId) === Number(user?.id) ? <button className="rc-history-remove" onClick={() => remover(share)} title="Remover do meu histórico" aria-label="Remover do meu histórico"><i aria-hidden="true" className="bi bi-trash" /></button> : <i className="bi bi-share-fill rc-history-icon" aria-hidden="true" />}
        </article>)}</div>}
      </section>
    </main>
    <Footer />
