...
- `GET /boards/analytics/export`: requer `dashboard.full.view` e `dashboard.export`; exporta em XLSX os registros do recorte analítico, respeitando categoria, setor e período.
...
- Contas com `dashboard.export` podem baixar uma planilha XLSX do board analítico. O arquivo utiliza os filtros aplicados, preserva acentos, dimensiona as colunas, fixa o cabeçalho, oferece autofiltro e apresenta datas no horário de Cotia. Campos privados de revisão de censura não são incluídos.
